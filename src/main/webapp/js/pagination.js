function generatePagination(container, listFunction, maxResults, query, page, limit) {
	var pages = Math.ceil(maxResults / limit);
	
	$(container).empty();
	// var prevClazz = (page-1) == 0 ? 'disabled' : '';
	
	if ((page-1) == 0) {
		$(container).append('<li class="page-item disabled"><a class="page-link">Previous</a></li>');
	} else {
	...
	if (page == pages) {
		$(container).append('<li class="page-item disabled"><a class="page-link">Next</a></li>');
	} else {
